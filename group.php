...
if (isset($_SESSION['session_id'])) {
    ...
    if (isset($_SESSION['session_codice'])) {
        $_SESSION['session_type_group'] = "group.php";
        ...
        $counter = 0;
        //se l'utente corrente non è un amministratore viene negato l'accesso alla pagina
        ...
        //per ogni membro del gruppo viene eseguito un iterazioni di questo ciclo
...
<head>
    ...
    <script type="text/javascript" src="javascript/slide_down.js"></script>
...
<body>
    ...
    <!-- popup che appare alla pressione del bottone,per creare un nuovo post-->
            ...
            <section class="condominio">
                <div class="form">
                    <form id="form1" action="php/invite.php" method="post" enctype="multipart/form-data">
                        ...
                        <button id="submit" type="submit" class="submit-btn btn  btn-lg" name="send">Invita</button>
                    </form>
                </div>
            </section>
        </div>
    ...
    <!--amministratore può espellere membri-->
    <div class="overlay" id="overlay-espelli"></div>
    ...
    <!--amministratore può mandare email ad un membro-->
    <div class="overlay popup-mss" id="overlay-send-message"></div>

    <!--amministratore può mandare email a tutti i membri del gruppo-->
    <div class="overlay popup-all" id="overlay-send-message-all"></div>
    ...
    <div class="container people" id="people">
        ...
        <script>
            (function() {
                ...
                //aggiunto al div l'amministratore del gruppo(l'utente attuale visto che il file è group.php con i privilegi dell'amministratore) con l'immagine profilo e un bottone che tramite la funzione openModalAll permette di inviare una mail a tutti i membri del gruppo.  
                newNode.innerHTML = '<div class="person"><a class="image-link" href="profilo/images/' + amministratore[1] + '"><img src="profilo/images/' + amministratore[1] + '" class="img-fluid" alt="foto profilo"></a><p>' + amministratore[0] + ' <b>(Amministratore)</b><button type="button" class="btn btn-outline-warning reply-all-button" onclick="OpenModalAll();"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-reply-all-fill" viewBox="0 0 16 16"><path d="M8.021 11.9 3.453 8.62a.719.719 0 0 1 0-1.238L8.021 4.1a.716.716 0 0 1 1.079.619V6c1.5 0 6 0 7 8-2.5-4.5-7-4-7-4v1.281c0 .56-.606.898-1.079.62z"/><path d="M5.232 4.293a.5.5 0 0 1-.106.7L1.114 7.945a.5.5 0 0 1-.042.028.147.147 0 0 0 0 .252.503.503 0 0 1 .042.028l4.012 2.954a.5.5 0 1 1-.593.805L.539 9.073a1.147 1.147 0 0 1 0-1.946l3.994-2.94a.5.5 0 0 1 .699.106z"/></svg> All</button></p></div>';
                ...
                for (var i = 0; i < size; i++) {
                    ...
                    //aggiunta la classe card all'elemento div per applicargli le caratteristiche delle card di bootstrap
                    newNode.className = 'card mb-3';
                    //aggiunto al div gli utenti del gruppo con la loro immagine profilo e due bottoni visibili all'utente corrente perchè è l'amministratore,uno che tramite OpenModalQ permette di espellere il membro e il secondo tramite OpenModalMss permette di inviare una mail al membro corrispondente.
                    newNode.innerHTML = '<div class="post"><a class="image-link" href="profilo/images/' + people[i][1] + '"><img src="profilo/images/' + people[i][1] + '" class="img-fluid" alt="foto profilo"></a><p>' + people[i][0] + '<button type="button" class="btn btn-outline-danger expell-button" onclick="OpenModalQ(\'' + people[i][0] + '\');"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash3" viewBox="0 0 16 16"><path d="M6.5 1h3a.5.5 0 0 1 .5.5v1H6v-1a.5.5 0 0 1 .5-.5ZM11 2.5v-1A1.5 1.5 0 0 0 9.5 0h-3A1.5 1.5 0 0 0 5 1.5v1H2.506a.58.58 0 0 0-.01 0H1.5a.5.5 0 0 0 0 1h.538l.853 10.66A2 2 0 0 0 4.885 16h6.23a2 2 0 0 0 1.994-1.84l.853-10.66h.538a.5.5 0 0 0 0-1h-.995a.59.59 0 0 0-.01 0H11Zm1.958 1-.846 10.58a1 1 0 0 1-.997.92h-6.23a1 1 0 0 1-.997-.92L3.042 3.5h9.916Zm-7.487 1a.5.5 0 0 1 .528.47l.5 8.5a.5.5 0 0 1-.998.06L5 5.03a.5.5 0 0 1 .47-.53Zm5.058 0a.5.5 0 0 1 .47.53l-.5 8.5a.5.5 0 1 1-.998-.06l.5-8.5a.5.5 0 0 1 .528-.47ZM8 4.5a.5.5 0 0 1 .5.5v8.5a.5.5 0 0 1-1 0V5a.5.5 0 0 1 .5-.5Z"></path></svg>Espelli</button><button type="button" class="btn btn-outline-warning Mss-button" onclick="OpenModalMss(\'' + people[i][0] + '\');"> <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-envelope-plus" viewBox="0 0 16 16"><path d="M2 2a2 2 0 0 0-2 2v8.01A2 2 0 0 0 2 14h5.5a.5.5 0 0 0 0-1H2a1 1 0 0 1-.966-.741l5.64-3.471L8 9.583l7-4.2V8.5a.5.5 0 0 0 1 0V4a2 2 0 0 0-2-2H2Zm3.708 6.208L1 11.105V5.383l4.708 2.825ZM1 4.217V4a1 1 0 0 1 1-1h12a1 1 0 0 1 1 1v.217l-7 4.2-7-4.2Z"/><path d="M16 12.5a3.5 3.5 0 1 1-7 0 3.5 3.5 0 0 1 7 0Zm-3.5-2a.5.5 0 0 0-.5.5v1h-1a.5.5 0 0 0 0 1h1v1a.5.5 0 0 0 1 0v-1h1a.5.5 0 0 0 0-1h-1v-1a.5.5 0 0 0-.5-.5Z"/></svg></button></p></div>';
