<?php
//file che si occupa di mandare un email ad ogni membro del gruppo.
session_start();
...
if (isset($_SESSION['session_id'])) {
    ...
    if (isset($_POST['send-all'])) {
        $message = $_POST['messaggio'];
        $nome = $_POST['nome'];

        //query che recupera l'username degli utenti del gruppo
        $query = "
            SELECT username
            FROM groups
        ...
        foreach ($result as $row) {
            //query che recupera l'email di un membro del gruppo
            $query = "
                SELECT email
                FROM users
